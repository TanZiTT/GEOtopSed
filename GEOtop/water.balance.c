...

{
	...
	hrill=new_doublematrix(Nr,Nc);
    initialize_doublematrix(hrill,0);
  ...
  /*
	for(r=1;r<=Nr;r++){
		for(c=1;c<=Nc;c++){
		   //temp=h[r][c]*dx*dy/w_dx_l/b[top->DD->co[r][c]+1]/b[top->DD->co[r][c]+1];
		   if(h[r][c]>0 && land->LC->co[r][c]!=NoV){
		   hrill->co[r][c]=h[r][c]*dx*dy/w_dx_l/b[top->DD->co[r][c]+1]/b[top->DD->co[r][c]+1];
		   //printf("hrill=%f, h[%d][%d]=%f,dx=%f,dy=%f,w_dx_l=%f,b=%f\n",hrill->co[r][c],r,c,h[r][c],dx,dy,w_dx_l,b[top->DD->co[r][c]+1]);
	   	   // hrill[r][c]=h[r][c]*dx*dy/w_dx_l/b[top->DD->co[r][c]+1]/b[top->DD->co[r][c]+1];
		   }
		}
    }
	*/
	...
	for(r=1;r<=Nr;r++){
		for(c=1;c<=Nc;c++){
			if(h[r][c]>0 && land->LC->co[r][c]!=NoV){
			    if(land->LC->co[r][c]<10){
				hrill->co[r][c]=h[r][c]*dx*dy/w_dx_l/b[top->DD->co[r][c]+1]/b[top->DD->co[r][c]+1];
				}else{hrill->co[r][c]=h[r][c];}
			}		
        }
    }
		...
		do{
			...
			find_dt_max(max_Courant_sup, hrill->co, land, top, cnet, par, &dt);
				...
				for(c=1;c<=Nc;c++){
					if(land->LC->co[r][c]!=NoV){
						...
						if(h[r][c] > 0){
							...
							Ks=cm_h(land->ty->co[lu][jcm], hrill->co[r][c], par->thres_hsup);
							R=r+r_DD[top->DD->co[r][c]];
							C=c+c_DD[top->DD->co[r][c]];
							if(land->LC->co[R][C]!=NoV){
							
								i=0.001*( hrill->co[r][c] - hrill->co[R][C])/b[top->DD->co[r][c]+1] + top->i_DD->co[r][c];				
								if(i<0) i=0.0;							
								q = b[top->DD->co[r][c]+1]*w_dx_l*Ks*pow(hrill->co[r][c]/1000.0,1.0+par->gamma_m)*sqrt(i)*1000.0/dx/dy;
						...
						dh[r][c] = Fmin( q*dt*dx*dy/w_dx_l/b[top->DD->co[r][c]+1]/b[top->DD->co[r][c]+1], Fmax( hrill->co[r][c], 0.0 ) );
			...
			for(r=1;r<=Nr;r++){
				for(c=1;c<=Nc;c++){
					if(land->LC->co[r][c]!=NoV){
						...
						hrill->co[r][c] = hrill->co[r][c]-dh[r][c];
						...
						if(land->LC->co[R][C]!=NoV){
							...
							if(hrill->co[R][C]>0){
								...
								if( dh[r][c] > 0) hrill->co[R][C] = dh[r][c];
							}
			...
			for(ch=1;ch<=cnet->r->nh;ch++){
				r = cnet->r->co[ch];
				c = cnet->c->co[ch];
				printf("channel: h[%d][%d]=%f,hsup[%d]=%f, depr=%f\n",r,c,h[r][c],ch,cnet->h_sup->co[ch],par->depr_channel);
				if(r>0){
	...
	for(r=1;r<=Nr;r++){
		for(c=1;c<=Nc;c++){
			if(h[r][c]>0 && land->LC->co[r][c]!=NoV){
				...
				q=b[top->DD->co[r][c]+1]*w_dx_l*Ks*pow(h[r][c]/1000.0,1.0+par->gamma_m)*sqrt(i)*1000.0/dx/dy;	//mm/s
				if(q>0){
	...
	if( par->point_sim!=1 && cnet->r->co[1]!=0 ){	//if it is not point simulation and there are channels
		...
		do{
			...
			for(ch=1;ch<=cnet->r->nh;ch++){
				...
				// printf("length=%f,dx=%f,dy=%f\n", cnet->length->co[ch],UV->U->co[1],UV->U->co[2]);  //TZ
			}
